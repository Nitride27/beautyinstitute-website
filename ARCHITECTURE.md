...
3. **About "Meet the Trainers" / course-detail certificate badge — "Momentum in Orbit" rotation.** A circular cropped image (trainer group photo, or a circular "Certified" badge on Course Detail) slowly rotates as the section scrolls through view — a slow, continuous `rotate` scrubbed to scroll position, capped well below the reference's full-spin speed to stay editorial rather than playful.
